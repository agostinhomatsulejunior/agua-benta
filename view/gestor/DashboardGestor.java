...
public class DashboardGestor extends JPanel {
    public DashboardGestor(Utilizador gestorLogado) {
        setLayout(null);
        setBackground(Estilos.COR_FUNDO);

        // Título com saudação personalizada
        JLabel titulo = new JLabel("Dashboard do Gestor - Bem-vindo, " + gestorLogado.getNome());
        titulo.setFont(Estilos.FONTE_TITULO);
        titulo.setForeground(Estilos.COR_TEXTO);
        titulo.setBounds(30, 20, 600, 30);
        add(titulo);

        // Estatísticas (todas globais, mas pode adaptar para apenas do gestor se desejar)
